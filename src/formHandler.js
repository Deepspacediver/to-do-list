...
function clearForm(button) {
  const inputContainers = Array.from(
    button.closest("div").querySelector("fieldset").children
  );
  inputContainers.forEach((div) => {
    Array.from(div.querySelectorAll("input")).forEach((input) => {
      //Collects all input within all divs
      input.value = "";
    });
  });
}
...
function createTodoDiv(todoObject) {
  ...
  todoDate.textContent = format(new Date(todoObject.date), "dd-MMMM-yyyy")
    .replace(/-/g, " ")
    .toUpperCase(); //replaces hyphens with spaces
  ...
  editTodoButton.addEventListener("click", function (e) {
    if (e.target.dataset.projectId == selectedProjectId) {
      ...
      let selectedTodoWrapper = document.querySelector(
        `div[data-todo-id="${selectedTodoId}"]`
      );
...
function removeTodo(target) {
  ...
  if (selectedProjectId == 0) {
    indexOfTodoInArray = todoArray.findIndex(
      (todo) => todo.originID == idOfTodo
    );
    todoArray.splice(indexOfTodoInArray, 1);
    console.log({ indexOfTodoInArray });
  } else {
    indexOfTodoInArray = projectList.findIndex(
      (project) => project.todos.originID == idOfTodo
    );
...
function setIdOfSelectedTodo(target) {
  localStorage.setItem(LOCAL_STORAGE_SELECTED_TODO_KEY, target.dataset.todoId);
}
